...
casper.thenOpen('http://localhost:3000', function (result) {
  ...
  casper.then(function(){
    casper.test.assertSelectorHasText('p', 'Hi, What would you like to know about your portfolio');

    // Enter -  Request
    casper.sendKeys('#textInputOne', 'list my portfolio');
 ...
 // Process response
  casper.then(function () {
   ...
   //Check for Response
   casper.then(function(){
    casper.test.assertSelectorHasText('p', 'What is the impact on my portfolio if the S&P 500 goes down 5% ');
   });
